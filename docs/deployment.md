...
Keep the app bound to loopback. Your HTTPS reverse proxy should forward to `127.0.0.1:8788`, preserve `Host` and `Origin`, terminate TLS, and impose suitable request limits. Set `TRUSTED_PROXIES=127.0.0.1/32,::1/128` for this loopback proxy, as in the runtime template. The default is empty (no proxy trust), appropriate for direct exposure. Only explicit IP addresses and CIDR subnets are accepted; never use a wildcard, `true`, or a broad network containing untrusted clients. The proxy must overwrite incoming `X-Forwarded-For` with the actual client address (for nginx: `proxy_set_header X-Forwarded-For $remote_addr;`), not preserve a client-supplied chain. Keep the app bound to loopback so only the trusted proxy can reach it. Express walks forwarded addresses from the socket toward the client and stops at the first untrusted hop. Each resulting client has its own authentication bucket (25 attempts per 15 minutes), shared by password and Google endpoints. For additional proxy hops, explicitly trust only operator-controlled addresses and configure each hop to sanitize forwarding headers. Set `PUBLIC_ORIGIN` to the exact HTTPS origin without a trailing slash. It controls callback URLs and mutation-origin checks. HTTPS origins automatically enable Secure cookies; HTTP origins are accepted only for loopback development. Configure proxy access logs to omit query strings, especially on `/api/auth/google/callback`, so authorization codes are not recorded. The app itself does not log requests, tokens, or answers. Review community moderation and abuse-handling needs before exposing a self-hosted dating service.
...
Authenticated JSON mutations require `X-Expected-Member` with the opaque `mutationContext` returned by `/api/me` when the action or draft was created. A missing or mismatched identity returns 409; the session cookie remains the authority. The context binds both member and session, so a deleted and reused numeric ID cannot accept old drafts. Google linking also binds its start URL to that context. Browser drafts stay in memory, survive internal navigation, and clear on account or demo changes; reloading the page discards unsaved drafts.
