...
The 79 historical questions have two to four documented answer options. A member saves:
...
| [Mathias Gatti's question-only CSV, fixed January 21, 2022 revision](https://github.com/mathigatti/okCupidScraper/blob/47967ab9745e13300b46a7ead2e5c0d1b02fe216/questions.csv) | **3,318 question records** with choice arrays. 54 prompts match the official 2011 chart after case/punctuation/spacing normalization. | Establishes choices in this later file, not in 2011. We did not run the scraper or download member/profile/response datasets. The repository carries MIT terms; see [third-party notices](../THIRD_PARTY_NOTICES.md). Those terms do not establish ownership of every historical question. |

The app now contains **79 historical prompts with documented choices**. It opens the historical collection by default and offers a stricter “Prompt + choices from 2011” collection. Search operates within the selected collection, across topics. The per-question provenance in `shared/historical.js` and `shared/questions.js` includes prompt and choice sources and distinguishes:
...
- **51 questions with later choices**, ID 162 and IDs 192–241: prompts witnessed in the 2011 article, choices from the 2022 file. Two of the 54 overlapping prompts (“Are you happy with your life?” and the dice question) instead use their earlier listing choices. ID 162 already had the same Yes/No ordering and did not need an answer change.


The chart count includes four prompts with escaped quotation marks. One additional chart/CSV overlap is outside this selected subset. The distinctions are substantive: the 2011 listing gives “Are you happy with your life?” three options, including “Most of the time.”, whereas the 2022 file has only Yes/No. We do not infer original options merely because a prompt looks binary. The unpaired chart prompts are research findings, not silently converted into invented historical questions. Dated wording, including the listing's “descent yard” typo, is preserved. Topic grouping is an okaycupid choice.

Active IDs 162 and 164–241 retain their prompt and option orderings. IDs 1–161 and 163 are retired: their wording is absent from the current source catalog, and their records are excluded from answering, progress, matching and comparisons. Stored owner answers and skip markers remain untouched and are included in exports with retirement metadata. No database data or Git history is rewritten. The 166 chart entries and the later 3,318-row file are **not a recovered complete original questionnaire**. The app uses only this attributed, cross-checked subset; raw research remains outside Git. No matching weights are inferred from the article's correlation claims.
